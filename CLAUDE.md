...
| **Nav** | `gfunnel/nav/` | `BxGfNav` / `gfn_` | **DB-backed chrome** shape: the GFunnel-owned left navigation (sidebar). `gfn_items` (global defaults) + `gfn_user_items` (per-member overrides). Rendered by `BxBaseFunctions::getGfSidebar()` (always-loaded, DB-or-defaults) via the `gf_sidebar` component key in `pt_application.html`; `serviceSaveMenu` handles per-member edits. Tables via `custom_batch_updates/gf_nav_install.php`. See `docs/gfunnel-left-nav.md`. |
...
| `docs/gfunnel-left-nav.md` | GFunnel-owned left navigation / sidebar (`gfunnel/nav` module + `BxBaseFunctions::getGfSidebar()`) |
