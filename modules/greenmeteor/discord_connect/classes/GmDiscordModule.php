...
class GmDiscordModule extends BxBaseModConnectModule
{
    ...
    function actionStart()
    {
        ...
        if (!$this->_oConfig->sApiID || !$this->_oConfig->sApiSecret) {
            ...
            $sCode =  MsgBox( _t('_gm_discord_profile_error_api_keys') );
            $this->_oTemplate->getPage(_t('_gm_discord'), $sCode);
        } 
        else {

            // define redirect URL to the remote site                
            $sUrl = bx_append_url_params($this->_oConfig->sOauthUrl . '/authorize', [
                'response_type' => 'code',
    ...
    function actionHandle()
    {
        ...
        $s = bx_file_get_contents($this->_oConfig->sApiUrl . '/oauth2/token', [
            'client_id'     => $this->_oConfig->sApiID,
        ...
        $s = bx_file_get_contents($this->_oConfig->sApiUrl . '/users/@me', [], 'get', [
    ...
    protected function _convertRemoteFields($aProfileInfo, $sAlternativeName = '')
    {
        ...
        $aProfileFields['allow_view_to'] = getParam('gm_discord_privacy');
