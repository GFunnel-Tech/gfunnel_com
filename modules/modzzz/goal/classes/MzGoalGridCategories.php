...
class MzGoalGridCategories extends BxTemplGrid
{
    protected $_sModule;
    protected $_oModule;
    ...
    public function performActionAdd()
    {
        ...
        if($oForm->isSubmittedAndValid()) {
            ...
            BxDolForm::setSubmittedValue('title', '_mz_goal_cat_' . $sName, $oForm->aFormAttrs['method']);
            BxDolForm::setSubmittedValue('text', '_mz_goal_cat_' . $sName, $oForm->aFormAttrs['method']);
 ...
 //BEGIN UNA 13 RC
        ...
        if($oForm->getCleanValue('type_clone') == 'on') {
                ...
                $aTypeNew = array('name' => $sTypeNewName, 'title' => '_mz_goal_cat_type_' . $sTypeNewName);
                ...
                foreach($aDisplayTypes as $sDisplayType) {
                    ...
                    $sDisplayNewName = 'mz_goal_entry_' . $sTypeNewName . '_' . $sDisplayType;
                    $sDisplayNewTitle = '_mz_goal_form_display_' . $sTypeNewName . '_' . $sDisplayType;
 ...
 //END UNA 13 RC
            ...
            if($iId != 0)
                ...
                $aRes = array('msg' => _t('_mz_goal_grid_action_err_add_category'));

            echoJson($aRes);
        }
        else {
            $sContent = BxTemplStudioFunctions::getInstance()->popupBox($this->_sModule . '-category-add-popup', _t('_mz_goal_grid_popup_title_add'), $this->_oModule->_oTemplate->parseHtmlByName('category_form.html', array(
    ...
    public function performActionEdit()
    {
        ...
        if($oForm->isSubmittedAndValid()) {
            ...
            if($iId != 0)
                ...
                $aRes = array('msg' => _t('_mz_goal_grid_action_err_edit_category'));

            echoJson($aRes);
        }
        else {
            $sContent = BxTemplStudioFunctions::getInstance()->popupBox($this->_sModule . '-category-edit-popup', _t('_mz_goal_grid_popup_title_edit', _t($aCategory['title'])), $this->_oModule->_oTemplate->parseHtmlByName('category_form.html', array(
                'form_id' => $oForm->aFormAttrs['id'],
                'form' => $oForm->getCode(true),
                'object' => $this->_sObject,
                'action' => $sAction
    ...
    protected function _getFormObject($sAction, $aCategory = array())
    {
        ...
        if(isset($oForm->aInputs['parent_id'])) {
            if(empty($this->_iParentId)) {
                $oForm->aInputs['parent_id']['values'][] = array('key' => 0, 'value' => _t('_mz_goal_form_category_input_parent_id_empty'));
    ...
    {
        $sContent = _t('_mz_goal_grid_column_value_subcategories', $this->_oModule->_oDb->getCategories(array('type' => 'parent_id_count', 'parent_id' => $aRow['id'])));

        $mixedValue = $this->_oTemplate->parseLink(bx_append_url_params($this->_sBaseUrl, array('parent_id' => $aRow['id'])), $sContent, array(
            'title' => _t('_mz_goal_grid_column_info_subcategories')
        ));
