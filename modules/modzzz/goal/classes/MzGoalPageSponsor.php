...
class MzGoalPageSponsor extends BxTemplPage
{
	protected $MODULE;
    protected $_oModule;

    protected $_aEntryInfo;
    protected $_aSponsorInfo;
    ...
    public function __construct($aObject, $oTemplate = false)
    {
        ...
        if ($iSponsorId)
            $this->_aSponsorInfo = $this->_oModule->_oDb->getSponsorInfoById($iSponsorId);

        if (!empty($this->_aSponsorInfo[$CNF['FIELD_GOAL_ID']]))
            $this->_aEntryInfo = $this->_oModule->_oDb->getContentInfoById($this->_aSponsorInfo[$CNF['FIELD_GOAL_ID']]);
		...
		if ($this->_aEntryInfo) {
            ...
            $oMenuSubmenu->setObjectSubmenu($CNF['OBJECT_MENU_SUBMENU_VIEW_SPONSOR'], array (
                'title' => $sTitle,
                'link' => $sUrl,
                'icon' => $CNF['ICON'],
            ));
	...
	public function getCode ()
    {
        ...
        if (!$this->_aEntryInfo || !$this->_aSponsorInfo) {
