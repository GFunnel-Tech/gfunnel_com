...
class MzListingPageAnnouncement extends BxTemplPage
{
	protected $MODULE;
    protected $_oModule;
    ...
    public function __construct($aObject, $oTemplate = false)
    {
        ...
        if ($iMenuId)
            $this->_aMenuInfo = $this->_oModule->_oDb->getAnnouncementInfoById($iMenuId);

        if (!empty($this->_aMenuInfo[$CNF['FIELD_LISTING_ID']]))
            $this->_aEntryInfo = $this->_oModule->_oDb->getContentInfoById($this->_aMenuInfo[$CNF['FIELD_LISTING_ID']]);
		...
		if ($this->_aEntryInfo) {
            ...
            $oMenuSubmenu->setObjectSubmenu($CNF['OBJECT_MENU_SUBMENU_VIEW_ANNOUNCEMENT'], array (
                'title' => $sTitle,
                'link' => $sUrl,
                'icon' => $CNF['ICON'],
            ));
