...
class MzListingFormReward extends BxBaseModTextFormEntry
{
    public function __construct($aInfo, $oTemplate = false)
    {
        ...
        if(isset($this->aInputs[$CNF['FIELD_PHOTO']])) {
            $this->aInputs[$CNF['FIELD_PHOTO']]['storage_object'] = $CNF['OBJECT_STORAGE_REWARD'];
            $this->aInputs[$CNF['FIELD_PHOTO']]['images_transcoder'] = $CNF['OBJECT_IMAGES_TRANSCODER_PREVIEW_REWARD'];
    ...
    function initChecker ($aValues = array (), $aSpecificValues = array())
    {
        ...
        if (isset($this->aInputs[$CNF['FIELD_PHOTO']])) {
            ...
            if ($aValues && !empty($aValues['id'])) {
                $aRewardInfo = $this->_oModule->_oDb->getRewardInfoById ($aValues['id']);
                $this->aInputs[$CNF['FIELD_PHOTO']]['content_id'] = $aValues['id'];
            }

            $this->aInputs[$CNF['FIELD_PHOTO']]['ghost_template'] = $this->_oModule->_oTemplate->parseHtmlByName('form_ghost_template_subitem.html', $this->_getPhotoGhostTmplVars($aRewardInfo));
        }
...
*/ 
        if (!empty($CNF['OBJECT_COMMENTS_REWARD'])) {
            $o = BxDolCmts::getObjectInstance($CNF['OBJECT_COMMENTS_REWARD'], $iContentId);
