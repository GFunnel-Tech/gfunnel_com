...
class MzListingFormGift extends BxBaseModTextFormEntry
{
    public function __construct($aInfo, $oTemplate = false)
    {
        ...
        if(isset($this->aInputs[$CNF['FIELD_PHOTO']])) {
            $this->aInputs[$CNF['FIELD_PHOTO']]['storage_object'] = $CNF['OBJECT_STORAGE_GIFT'];
            $this->aInputs[$CNF['FIELD_PHOTO']]['images_transcoder'] = $CNF['OBJECT_IMAGES_TRANSCODER_PREVIEW_GIFT'];
    ...
    function initChecker ($aValues = array (), $aSpecificValues = array())
    {
        ...
        if (isset($this->aInputs[$CNF['FIELD_PHOTO']])) {
            ...
            if ($aValues && !empty($aValues['id'])) {
                $aGiftInfo = $this->_oModule->_oDb->getGiftInfoById ($aValues['id']);
                $this->aInputs[$CNF['FIELD_PHOTO']]['content_id'] = $aValues['id'];
            }

            $this->aInputs[$CNF['FIELD_PHOTO']]['ghost_template'] = $this->_oModule->_oTemplate->parseHtmlByName('form_ghost_template_subitem.html', $this->_getPhotoGhostTmplVars($aGiftInfo));
        }
...
*/ 
        if (!empty($CNF['OBJECT_COMMENTS_GIFT'])) {
            $o = BxDolCmts::getObjectInstance($CNF['OBJECT_COMMENTS_GIFT'], $iContentId);
