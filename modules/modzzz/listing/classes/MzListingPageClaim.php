...
class MzListingPageClaim extends BxTemplPage
{
	protected $MODULE;
    protected $_oModule;
    ...
    public function __construct($aObject, $oTemplate = false)
    {
        ...
        if ($iClaimId)
            $this->_aClaimInfo = $this->_oModule->_oDb->getClaimInfoById($iClaimId);

        if (!empty($this->_aClaimInfo[$CNF['FIELD_LISTING_ID']]))
            $this->_aEntryInfo = $this->_oModule->_oDb->getContentInfoById($this->_aClaimInfo[$CNF['FIELD_LISTING_ID']]);
		...
		if ($this->_aEntryInfo) {
            ...
            $oMenuSubmenu->setObjectSubmenu($CNF['OBJECT_MENU_SUBMENU_VIEW_CLAIM'], array (
                'title' => $sTitle,
                'link' => $sUrl,
                'icon' => $CNF['ICON'],
            ));
	...
	public function getCode ()
    {
        ...
        if (!$this->_aEntryInfo || !$this->_aClaimInfo) {
            $this->_oTemplate->displayPageNotFound();
            exit;
        }
 
        // permissions check 
        if (CHECK_ACTION_RESULT_ALLOWED !== ($sMsg = $this->_oModule->checkAllowedManageClaim($this->_aClaimInfo))) {
