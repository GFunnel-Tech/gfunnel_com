...
class MzListingPageReward extends BxTemplPage
{
	protected $MODULE;
    protected $_oModule;

    protected $_aEntryInfo;
    protected $_aRewardInfo;
    ...
    public function __construct($aObject, $oTemplate = false)
    {
        ...
        if ($iRewardId)
            $this->_aRewardInfo = $this->_oModule->_oDb->getRewardInfoById($iRewardId);

        if (!empty($this->_aRewardInfo[$CNF['FIELD_LISTING_ID']]))
            $this->_aEntryInfo = $this->_oModule->_oDb->getContentInfoById($this->_aRewardInfo[$CNF['FIELD_LISTING_ID']]);
		...
		if ($this->_aEntryInfo) {
            ...
            $oMenuSubmenu->setObjectSubmenu($CNF['OBJECT_MENU_SUBMENU_VIEW_REWARD'], array (
                'title' => $sTitle,
                'link' => $sUrl,
                'icon' => $CNF['ICON'],
            ));
	...
	public function getCode ()
    {
        ...
        if (!$this->_aEntryInfo || !$this->_aRewardInfo) {
