 ...
 */
class MzNewsFormEntry extends BxBaseModTextFormEntry
{
    ...
    public function __construct($aInfo, $oTemplate = false)
    {
    	...
    	if(isset($CNF['FIELD_COVER']) && isset($this->aInputs[$CNF['FIELD_COVER']])) {
            if($this->_oModule->checkAllowedSetThumb() === CHECK_ACTION_RESULT_ALLOWED) {
                ...
                $this->aInputs[$CNF['FIELD_COVER']]['upload_buttons_titles'] = array(
                    'Simple' => _t('_mz_news_form_entry_input_covers_uploader_simple_title'), 
                    'HTML5' => _t('_mz_news_form_entry_input_covers_uploader_html5_title')
                );
        ...
        if(isset($this->aInputs[$CNF['FIELD_LANGUAGE']])) {  
			if ($this->_oModule->_oConfig->_isMultiLanguage)  
				$this->aInputs[$CNF['FIELD_LANGUAGE']]['values'] = $this->getLanguages();
			else 
				unset($this->aInputs[$CNF['FIELD_LANGUAGE']]); 
		}
 
        if (isset($this->aInputs[$CNF['FIELD_BREAKING_NEWS']]) && !$this->isAdmin()) {
            unset($this->aInputs[$CNF['FIELD_BREAKING_NEWS']]);
        } 
    }
 
    public function isAdmin($iProfileId = 0)
    { 
		$iProfileId = ($iProfileId) ? $iProfileId : bx_get_logged_profile_id ();
        return BxDolAcl::getInstance()->isMemberLevelInSet(array(MEMBERSHIP_ID_MODERATOR, MEMBERSHIP_ID_ADMINISTRATOR), $iProfileId);
    }
    ...
    protected function genCustomInputMulticat(&$aInput)
    {
		...
		//$aValuesForSelect = BxDolCategories::getInstance()->getData(array('type' => 'by_module_and_author', 'module' => $this->_oModule->getName(), 'author' => bx_get_logged_profile_id()));
   		$aValuesForSelect = BxDolCategories::getInstance()->getData(array('type' => 'by_module', 'module' => $this->_oModule->getName()));
