...
require_once('MsFansonlyGridRequests.php');

class MsFansonlyGridRequestsManage extends MsFansonlyGridRequests
{
    ...
    */
    ...
    public function getCode($isDisplayHeader = true)
    {
        ...
        $sJsCode = $this->_oModule->_oTemplate->getJsCode('requests', array(
    ...
    public function performActionAccept()
    {
        ...
        $aIdsAffected = array();
        foreach($aIds as $iId)
            if($this->_oModule->_oDb->updateRequest($iId, 'accepted')) {
                $aIdsAffected[] = $iId;
                $iAffected++;

                $aRequestSent = $this->_oModule->_oDb->getById($iId);
                $this->_oModule->triggerAlertAction($aRequestSent, 'doRequestaccepted');
                
            }

        return echoJson(array('grid' => $this->getCode(false), 'blink' => $aIdsAffected));

//        return echoJson($iAffected ? array('grid' => $this->getCode(false), 'blink' => $aIdsAffected) : array('msg' => _t('_ms_fansonly_err_cannot_perform')));
    }
    public function performActionIgnore()
    {
    	$CNF = &$this->_oModule->_oConfig->CNF;
        ...
        $aIdsAffected = array();
        foreach($aIds as $iId)
            if($this->_oModule->_oDb->updateRequest($iId, "ignored")) {
                $aIdsAffected[] = $iId;
                $iAffected++;
            }
...
//        return echoJson($iAffected ? array('grid' => $this->getCode(false), 'blink' => $aIdsAffected) : array('msg' => _t('_ms_fansonly_err_cannot_perform')));
    ...
    protected function _getActionIgnore ($sType, $sKey, $a, $isSmall = false, $isDisabled = false, $aRow = array())
    {
//        print json_encode($aRow, true);
        if(!empty($aRow["status"]) && $aRow["status"] == "ignored")
            return '';
...
//        print json_encode($aRow, true);
        ...
        if(!empty($aRow["status"]) && $aRow["status"] == "accepted")
            return '';
    ...
    protected function _addJsCss()
    {
        ...
        $this->_oModule->_oTemplate->addJs(array('jquery.form.min.js', 'requests.js'));
        $this->_oModule->_oTemplate->addCss(array('requests.css'));
    }
