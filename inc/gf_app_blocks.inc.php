...
/**
 * GFunnel — Application Hub: shared renderers, data helpers and the directory
 * importer. Included by the standalone SEO page (gf_applications.php) AND by the
 * gfunnel_applications module's Studio service block, so both stay in sync
 * (same pattern as inc/gf_home_blocks.inc.php).
 */
...
{
    $aMember = gfAppMember();
    $sInner = gfAppHubInner($oDb, $aMember, $sTab, false);
    ...
    if ($sTab === 'marketplace') {
        $aMeta = array(
            ...
            'canonical' => BX_DOL_URL_ROOT . 'marketplace/applications',
            'member' => $aMember,
        );
    } else {
        $aMeta = array(
            ...
            'canonical' => BX_DOL_URL_ROOT . 'applications',
            'member' => $aMember,
        );
    }
...
/**
 * The hub body — shared by the standalone SEO page and the module Studio block.
 *   $bBlock=true  → both panels in a self-contained box (.gfa-box) with
 *                   client-side tab toggle; used by the gfunnel_applications
 *                   service block so it renders inside the workspace shell.
 *   $bBlock=false → only the active panel inside .gfa-main (standalone page;
 *                   avoids loading the 5k-row directory on the Apps route).
 */
function gfAppHubInner($oDb, $aMember, $sTab = 'apps', $bBlock = false)
{
    $sTab = ($sTab === 'marketplace') ? 'marketplace' : 'apps';
    $aScope = $aMember['logged'] ? gfAppScope($aMember) : array(0, 0);
    $aMine = $aMember['logged'] ? gfWsAppIds($oDb, $aScope) : array();
    $aMineSet = array_flip($aMine);

    $sAdmin = gfAppAdminBar($aMember);
    ...
    $sPanel = ($sTab === 'marketplace')
        ? gfAppMarketplacePanel($oDb, $aMember, $aMineSet)
        : gfAppAppsPanel($oDb, $aMember, $aMine);
    return '<section class="gfa-main"><div class="gfh-container">' . $sAdmin . $sTabBar . $sPanel . '</div></section>';
}

/**
 * The Apps | Marketplace tab bar + Video Chat action.
 * $bBlock=true → client-side toggle buttons (module block, one page); else
 * route links (standalone /applications vs /marketplace/applications).
 */
function gfAppTabBar($sTab, $bBlock = false)
{
...
/** Apps panel content: welcome hero + Core Applications icon grid + hub cards. */
function gfAppAppsPanel($oDb, $aMember, $aMine = array())
{
...
/** Marketplace panel content: full App Directory (search + chips + grid + pager). */
function gfAppMarketplacePanel($oDb, $aMember, $aMineSet = array())
{
    ...
    return '<div class="gfa-sec" style="margin-bottom:0">'
